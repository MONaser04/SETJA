...
class InstantOverlay(QWidget):
    ...
    def show_context_menu(self, pos):
        ...
        settings_path = os.path.join(grandparent_dir, "settings.json")
        engine = "offline"
        api_key = ""
        ...
        offline_action = menu.addAction("Engine: Offline")
        offline_action.setCheckable(True)
        offline_action.setChecked(engine == "offline")
        
        gemini_action = menu.addAction("Engine: Gemini (Online)")
        gemini_action.setCheckable(True)
        gemini_action.setChecked(engine == "gemini")
        
        menu.addSeparator()
        key_action = menu.addAction("Set Gemini API Key...")
        
        action = menu.exec(pos)
        
        if action == offline_action:
            engine = "offline"
        elif action == gemini_action:
            engine = "gemini"
        elif action == key_action:
            new_key, ok = QInputDialog.getText(self, "Gemini API Key", "Enter API Key:", text=api_key)
            if ok:
        ...
        with open(settings_path, "w", encoding="utf-8") as f:
            json.dump({"engine": engine, "gemini_api_key": api_key, "gemini_model": model_name}, f, indent=4)
