...
def _get_gemini_key(fallback_key: str) -> str:
    global _GEMINI_KEY_INDEX
    try:
        import os
        root_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        key_path = os.path.join(root_dir, "api_key.txt")
    ...
    if os.path.exists(settings_path):
        try:
            with open(settings_path, "r", encoding="utf-8") as f:
                ...
                api_key_from_settings = s.get("gemini_api_key", "")
                model_name = s.get("gemini_model", "gemini-3.1-flash-lite")
        except:
            pass

    if engine == "gemini":
        api_key = _get_gemini_key(api_key_from_settings)
        if api_key:
            t0 = time.monotonic()
            try:
                url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_name}:generateContent?key={api_key}"
                payload = {
                    "contents": [{"parts":[{"text": f"Translate the following text to Arabic. Provide ONLY the translation, without any quotes or extra formatting:\\n\\n{text}"}]}],
                    "generationConfig": {"temperature": 0.3}
                }
                r = session.post(url, json=payload, timeout=cfg.mt_timeout)
                if r.status_code == 429:
                    _mark_gemini_key_failed()
                    return {"ok": False, "error": "Gemini API Quota Exceeded (429). Switching to next key...", "ms": 0.0}
                
                j = r.json()
                if "candidates" in j and j["candidates"]:
                    translated = j["candidates"][0]["content"]["parts"][0]["text"].strip()
                    ms = (time.monotonic() - t0) * 1000.0
                    return {"ok": True, "text": translated, "lines": [translated], "ms": ms}
                else:
                    if r.status_code != 200:
                        _mark_gemini_key_failed()
                    return {"ok": False, "error": f"Gemini API Error: {j}", "ms": 0.0}
            except Exception as e:
                return {"ok": False, "error": str(e), "ms": 0.0}
